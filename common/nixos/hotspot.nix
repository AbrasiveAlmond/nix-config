{pkgs, ... } : {
  ...
  systemd.services.hotspot = {
    ...
    # become = true;
    script = ''
      /run/current-system/sw/bin/nmcli r wifi off && /run/current-system/sw/bin/rfkill unblock wlan
      sudo ${pkgs.linux-wifi-hotspot}/bin/create_ap wlp13s0 enp14s0 'Minifridge' 'ColdBeers'
    '';
    # serviceConfig = {
